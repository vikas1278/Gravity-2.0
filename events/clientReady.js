...
module.exports = async (client) => {
    ...
    client.user.setPresence({
        activities: [{ name: 'Gravity Bot 3.0', type: ActivityType.Playing }],
        status: 'online',
    });

    client.errorLog = config.errorLog;

    // Preload message cache for AutoMod
    const { preloadMessageCache } = require('../utils/messageCache');
    preloadMessageCache(client).catch(err => console.error('Error preloading message cache:', err.message));
};
